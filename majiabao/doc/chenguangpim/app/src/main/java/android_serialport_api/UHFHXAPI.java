...
public class UHFHXAPI {
	...
	private class Packet {
		...
		public byte[] getPacket() {
			...
			if (payloadLength == 0) {
				commandPacket = Bytes.concat(new byte[][] { { messageType }, new byte[] { messageCode },
			...
			byte[] packet = Bytes.concat(new byte[][] { { preamble }, commandPacket, crc16 });
	...
	public boolean open() {
		...
		int requestLength = 1;
		int len = SerialPortManager.getInstance().readFixedLength(buffer, 3000, requestLength);
	 ...
	 * @return void
	 */
	public void startAutoRead2A(int commandCode, byte[] rc, final AutoRead autoRead) {
	...
	public Response readTypeCTagData(byte[] arguments) {
		...
		Response response = new Response();
		Log.d("jokey", "readTypeCTagData-->buffer:" + DataUtils.toHexString(buffer));
		if (length == 0) {
			response.result = Response.TIME_OUT;
		} else {
			if (buffer[2] != MessageCode.Read_Type_C_Tag_Data) {
				...
				Log.d("jokey", "readTypeCTagData-->data:" + DataUtils.toHexString(data));
				response.result = Response.RESPONSE_PACKET;
		...
		public synchronized byte[] getFullPacket() {
			...
			if (packetStartIndex < dataLength) {
				if (mBuffer[packetStartIndex] == (byte) 0xBB
						&& (mBuffer[packetStartIndex + 1] == UHFHXAPI.MessageType.Response
								|| mBuffer[packetStartIndex + 1] == UHFHXAPI.MessageType.Notification)) {
					...
					if (payloadLength >= 0 && mBuffer[packetStartIndex + 4 + payloadLength + 1] == 0x7E
						...
						Log.i("zzd", "tempBuf=" + DataUtils.toHexString(tempBuf));
					...
					packetStartIndex++;
					Log.i("zzd", "%%%%%%%%%%%packetStartIndex=" + packetStartIndex + "  dataLength=" + dataLength);
				}
			} else {
				packetStartIndex = 0;
				dataLength = 0;
